...
import Mentor from "../../Models/VirtualProduction.js";
  ...
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        { folder: "virtualproduction/mentors", resource_type: "image" },
...
if (doc) {
  doc.virtualProduction.mentor.push(newMentor); // ✅ correct
  await doc.save();
} else {
  doc = await Mentor.create({
    virtualProduction: { mentor: [newMentor] }, // ✅ fixed casing
  });
}
...
router.delete("/:publicId", async (req, res) => {
  try {
    ...
    const mentorToDelete = doc.virtualProduction.mentor.find(m => m.publicId === publicId);
    ...
    // Remove from MongoDB
    doc.virtualProduction.mentor = doc.virtualProduction.mentor.filter(m => m.publicId !== publicId);
    await doc.save();
