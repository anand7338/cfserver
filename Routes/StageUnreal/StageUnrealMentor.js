...
import Mentor from "../../Models/StageUnreal.js";
  ...
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        { folder: "stageunreal/mentors", resource_type: "image" },
...
if (doc) {
  doc.stageunreal.mentor.push(newMentor); // ✅ correct
  await doc.save();
} else {
  doc = await Mentor.create({
    stageunreal: { mentor: [newMentor] }, // ✅ fixed casing
  });
}
...
router.delete("/:publicId", async (req, res) => {
  try {
    ...
    const mentorToDelete = doc.stageunreal.mentor.find(m => m.publicId === publicId);
    ...
    // Remove from MongoDB
    doc.stageunreal.mentor = doc.stageunreal.mentor.filter(m => m.publicId !== publicId);
    await doc.save();
