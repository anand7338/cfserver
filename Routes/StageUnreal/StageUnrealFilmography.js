...
router.get("/", async (req, res) => {
  try {
    ...
    res.json(doc.stageunreal.filmography || [] );
  } catch (err) {
...
router.post("/upload", upload.single("image"), async (req, res) => {
  try {
    ...
    const streamUpload = () =>
      new Promise((resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream(
          { folder: "stageunreal/filmography" },
          (error, result) => {
    ...
    doc.stageunreal.filmography.push(newItem);
    await doc.save();
...
router.delete("/:publicId", async (req, res) => {
  try {
    ...
    // Delete from MongoDB
    const doc = await getOrCreateModel();
    doc.stageunreal.filmography = doc.stageunreal.filmography.filter((item) => item.publicId !== publicId);
    await doc.save();
