...
import Mentor from "../../Models/Editing.js";
  ...
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        { folder: "editing/mentors", resource_type: "image" },
    ...
    if (doc) {
      doc.editing.mentor.push(newMentor); // 👈 nested path
      await doc.save();
    } else {
      doc = await Mentor.create({
        editing: { mentor: [newMentor] }, // 👈 nested insert
      });
    }
...
router.delete("/:publicId", async (req, res) => {
  try {
    ...
    const mentorToDelete = doc.editing.mentor.find(m => m.publicId === publicId);
    ...
    // Remove from MongoDB
    doc.editing.mentor = doc.editing.mentor.filter(m => m.publicId !== publicId);
    await doc.save();
