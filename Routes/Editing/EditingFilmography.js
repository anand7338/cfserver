...
// ✅ Ensure a single Editing document exists
const getOrCreateEditing = async () => {
...
router.get("/", async (req, res) => {
  try {
    const doc = await getOrCreateEditing();
    res.json(doc.editing.filmography || [] );
  } catch (err) {
...
router.post("/upload", upload.single("image"), async (req, res) => {
  try {
    ...
    doc.editing.filmography.push(newItem);
    await doc.save();
...
router.delete("/:publicId", async (req, res) => {
  try {
    ...
    // Delete from MongoDB
    const doc = await getOrCreateEditing();
    doc.editing.filmography = doc.editing.filmography.filter((item) => item.publicId !== publicId);
    await doc.save();
