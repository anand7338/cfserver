...
import Mentor from "../../Models/Cinematography.js";
  ...
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        { folder: "cinematography/mentors", resource_type: "image" },
    ...
    if (doc) {
      doc.cinematography.mentor.push(newMentor); // 👈 nested path
      await doc.save();
    } else {
      doc = await Mentor.create({
        cinematography: { mentor: [newMentor] }, // 👈 nested insert
      });
    }
...
router.delete("/:publicId", async (req, res) => {
  try {
    ...
    const mentorToDelete = doc.cinematography.mentor.find(m => m.publicId === publicId);
    ...
    // Remove from MongoDB
    doc.cinematography.mentor = doc.cinematography.mentor.filter(m => m.publicId !== publicId);
    await doc.save();
