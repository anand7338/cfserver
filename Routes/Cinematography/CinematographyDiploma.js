...
async function getDiploma() {
  ...
  if (!cinematography.cinematography.diploma[0]) {
    cinematography.cinematography.diploma[0] = {
      semester1: [],
      semester2: [],
    }
    await cinematography.save()
  }
  return { cinematography, diploma: cinematography.cinematography.diploma[0] }
}

// GET Diploma Data
router.get("/", async (req, res) => {
  try {
    const { cinematography } = await getDiploma()
    res.json(cinematography)
  } catch (err) {
    console.error(err)
...
router.post("/text", async (req, res) => {
  try {
    ...
    const { cinematography, diploma } = await getDiploma()
...
// DELETE a subtitle
router.delete("/diploma/subtitle", async (req, res) => {
  try {
