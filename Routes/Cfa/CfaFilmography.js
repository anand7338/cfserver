...
router.post("/upload", upload.single("image"), async (req, res) => {
  try {
    ...
    doc.cfa.filmography.push(newItem);
    await doc.save();
...
router.delete("/:publicId", async (req, res) => {
  try {
    ...
    // Delete from MongoDB
    const doc = await getOrCreateModel();
    doc.cfa.filmography = doc.cfa.filmography.filter((item) => item.publicId !== publicId);
    await doc.save();
