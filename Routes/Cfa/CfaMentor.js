...
import Mentor from "../../Models/Cfa.js";
  ...
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        { folder: "cfa/mentors", resource_type: "image" },
...
if (doc) {
  doc.cfa.mentor.push(newMentor); // ✅ correct
  await doc.save();
} else {
  doc = await Mentor.create({
    cfa: { mentor: [newMentor] }, // ✅ fixed casing
  });
}
...
router.delete("/:publicId", async (req, res) => {
  try {
    ...
    const mentorToDelete = doc.cfa.mentor.find(m => m.publicId === publicId);
    ...
    // Remove from MongoDB
    doc.cfa.mentor = doc.cfa.mentor.filter(m => m.publicId !== publicId);
    await doc.save();
