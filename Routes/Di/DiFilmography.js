...
router.post("/upload", upload.single("image"), async (req, res) => {
  try {
    ...
    doc.di.filmography.push(newItem);
    await doc.save();
...
router.delete("/:publicId", async (req, res) => {
  try {
    ...
    // Delete from MongoDB
    const doc = await getOrCreateModel();
    doc.di.filmography = doc.di.filmography.filter((item) => item.publicId !== publicId);
    await doc.save();
