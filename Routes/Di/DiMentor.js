...
import Mentor from "../../Models/Di.js";
  ...
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        { folder: "di/mentors", resource_type: "image" },
...
if (doc) {
  doc.di.mentor.push(newMentor); // ✅ correct
  await doc.save();
} else {
  doc = await Mentor.create({
    di: { mentor: [newMentor] }, // ✅ fixed casing
  });
}
...
router.delete("/:publicId", async (req, res) => {
  try {
    ...
    const mentorToDelete = doc.di.mentor.find(m => m.publicId === publicId);
    ...
    // Remove from MongoDB
    doc.di.mentor = doc.di.mentor.filter(m => m.publicId !== publicId);
    await doc.save();
