...
// ✅ Ensure a single Direction document exists
const getOrCreateDirection = async () => {
...
router.get("/", async (req, res) => {
  try {
    const doc = await getOrCreateDirection();
    res.json(doc.direction.filmography || [] );
  } catch (err) {
...
router.post("/upload", upload.single("image"), async (req, res) => {
  try {
    ...
    doc.direction.filmography.push(newItem);
    await doc.save();
...
router.delete("/:publicId", async (req, res) => {
  try {
    ...
    // Delete from MongoDB
    const doc = await getOrCreateDirection();
    doc.direction.filmography = doc.direction.filmography.filter((item) => item.publicId !== publicId);
    await doc.save();
