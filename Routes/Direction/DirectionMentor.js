...
import Mentor from "../../Models/Direction.js";
  ...
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        { folder: "direction/mentors", resource_type: "image" },
    ...
    if (doc) {
      doc.direction.mentor.push(newMentor); // 👈 nested path
      await doc.save();
    } else {
      doc = await Mentor.create({
        direction: { mentor: [newMentor] }, // 👈 nested insert
      });
    }
...
router.delete("/:publicId", async (req, res) => {
  try {
    ...
    const mentorToDelete = doc.direction.mentor.find(m => m.publicId === publicId);
    ...
    // Remove from MongoDB
    doc.direction.mentor = doc.direction.mentor.filter(m => m.publicId !== publicId);
    await doc.save();
