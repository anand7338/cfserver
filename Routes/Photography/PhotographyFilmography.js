...
router.get("/", async (req, res) => {
  try {
    ...
    res.json(doc.photography.filmography || [] );
  } catch (err) {
...
router.post("/upload", upload.single("image"), async (req, res) => {
  try {
    ...
    const streamUpload = () =>
      new Promise((resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream(
          { folder: "photography/filmography" },
          (error, result) => {
    ...
    doc.photography.filmography.push(newItem);
    await doc.save();
...
router.delete("/:publicId", async (req, res) => {
  try {
    ...
    // Delete from MongoDB
    const doc = await getOrCreateModel();
    doc.photography.filmography = doc.photography.filmography.filter((item) => item.publicId !== publicId);
    await doc.save();
