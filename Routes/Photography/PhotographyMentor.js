...
import Mentor from "../../Models/Photography.js";
  ...
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        { folder: "photography/mentors", resource_type: "image" },
...
if (doc) {
  doc.photography.mentor.push(newMentor); // ✅ correct
  await doc.save();
} else {
  doc = await Mentor.create({
    photography: { mentor: [newMentor] }, // ✅ fixed casing
  });
}
...
router.delete("/:publicId", async (req, res) => {
  try {
    ...
    const mentorToDelete = doc.photography.mentor.find(m => m.publicId === publicId);
    ...
    // Remove from MongoDB
    doc.photography.mentor = doc.photography.mentor.filter(m => m.publicId !== publicId);
    await doc.save();
