...
import Mentor from "../../Models/Vfx.js";
  ...
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        { folder: "vfx/mentors", resource_type: "image" },
    ...
    if (doc) {
      doc.vfx.mentor.push(newMentor); // 👈 nested path
      await doc.save();
    } else {
      doc = await Mentor.create({
        vfx: { mentor: [newMentor] }, // 👈 nested insert
      });
    }
...
router.delete("/:publicId", async (req, res) => {
  try {
    ...
    const mentorToDelete = doc.vfx.mentor.find(m => m.publicId === publicId);
    ...
    // Remove from MongoDB
    doc.vfx.mentor = doc.vfx.mentor.filter(m => m.publicId !== publicId);
    await doc.save();
