...
// ✅ Ensure a single Vfx document exists
const getOrCreateVfx = async () => {
...
router.get("/", async (req, res) => {
  try {
    const doc = await getOrCreateVfx();
    res.json(doc.vfx.filmography || [] );
  } catch (err) {
...
router.post("/upload", upload.single("image"), async (req, res) => {
  try {
    ...
    doc.vfx.filmography.push(newItem);
    await doc.save();
...
router.delete("/:publicId", async (req, res) => {
  try {
    ...
    // Delete from MongoDB
    const doc = await getOrCreateVfx();
    doc.vfx.filmography = doc.vfx.filmography.filter((item) => item.publicId !== publicId);
    await doc.save();
